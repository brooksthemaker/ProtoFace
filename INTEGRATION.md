...
│
└── Protoface  (Python / numpy)   — drives 4× HUB75 LED panels
...
Per-frame work on the 128×64 canvas at 30 fps:
...
| HUB75 DMA transfer (rpi-rgb-led-matrix) | background thread |
| Shm write (24576 bytes memcpy) | < 0.1 ms |
...
ProtoHUD is CPU-light per frame (main cost is OpenGL ES calls and camera DMA).  The `ProtoFaceController` reconnect thread sleeps 2 s between polls — negligible.  ShmFrameReader polls at the render frame rate but only does a 24577-byte memcpy when the sequence byte changes.
...
| rpi-rgb-led-matrix DMA thread | 3 (or kernel-managed) |
...
**You cannot use the I2S microphone and the 4-panel parallel layout simultaneously.**
...
Alternatively, use the `adafruit-hat` mapping which uses different data lines — check against the SPI0 pins for your specific carrier board.
...
| `/dev/shm/protoface_frame` | 24 KB |
| rpi-rgb-led-matrix kernel buffers | ~2 MB |
| **Total** | ~200–400 MB |
...
| Display output | HDMI/DSI → HMD | HUB75 DMA (no GPU) |
| Camera ISP | 2× OV9281 cameras | Not used |

No GPU or display conflict.  Protoface renders entirely in CPU/numpy and pushes pixels via the `rpi-rgb-led-matrix` DMA engine, which bypasses the GPU completely.
...
> Both processes require **root** for HUB75 DMA and OpenGL ES display access.
...
|-------------|---------------|-----|
| HUB75 DMA (rpi-rgb-led-matrix) | Protoface needs root | `User=root` in service |
...
| GPIO 9+11: SPI0 IMU vs. HUB75 row-1 | **Blocking if ProtoHUD uses SPI IMU** | Move ProtoHUD IMU to I2C or use alternate HUB75 mapping |
...
4. Set `hardware_mapping: regular` and `gpio_slowdown: 4` in Protoface `config.yaml`
